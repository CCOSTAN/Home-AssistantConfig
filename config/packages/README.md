...
| [tesla_model_y.yaml](tesla_model_y.yaml) | Remind the garage and parents to plug in the Model Y after the large garage door closes with a low battery and charging still off, with an 8 PM repeat loop for recent garage arrivals. | `sensor.spaceship_battery_level`, `switch.spaceship_charge`, `cover.large_garage_door`, `input_datetime.tesla_model_y_last_garage_close`, `notify.alexa_media_garage`, `script.notify_engine` |
